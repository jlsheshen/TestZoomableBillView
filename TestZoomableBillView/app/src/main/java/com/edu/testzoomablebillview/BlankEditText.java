...
public class BlankEditText extends EditText implements IScaleable {
    ...
    public BlankEditText(Context context, AttributeSet attrs, int mTestMode, int mState) {
        super(context, attrs);
    ...
    private void initBlank(){
        ...
        //确认输入类型,在后来的SDK版本需要强制输入InputType的类型
        setInputType(InputType.TYPE_CLASS_TEXT);
        //判断是否需要用户填写,如果不需要则直接显示答案
        if (mData.isEditable()){
            setEnabled(true);
        }else {
            setEnabled(false);
